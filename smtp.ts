...
export async function sendEmail(
  ...
  try {
    ...
    if (!greeting.startsWith("220")) {
      throw new SmtpError(
        `SMTP server rejected connection: ${greeting.trim()}`,
      );
    }
    ...
    if (!authPrompt1.startsWith("334")) {
      throw new SmtpError(
        `SMTP AUTH failed (username prompt): ${authPrompt1.trim()}`,
      );
    }
    ...
    if (!authPrompt2.startsWith("334")) {
      throw new SmtpError(
        `SMTP AUTH failed (password prompt): ${authPrompt2.trim()}`,
      );
    }
    ...
    for (const rcpt of allRecipients) {
      ...
      if (!rcptResponse.startsWith("250")) {
        throw new SmtpError(
          `RCPT TO rejected for ${rcpt}: ${rcptResponse.trim()}`,
        );
      }
    }
